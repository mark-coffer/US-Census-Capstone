...
object QueryCensus{
    def main(args: Array[String]): Unit = {
       ...
       // Create DF from output10.csv
        ...
        val pop_2010 = spark.sql("SELECT SUM(totalPop) AS `2010 US Total_Pop` FROM states10").toDF()
       ...
       val pop_2020 = spark.sql("SELECT SUM(totalPop) AS `2020 US Total_Pop` FROM states20").toDF()
